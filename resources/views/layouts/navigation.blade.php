...
<aside :class="[sidebarOpen ? 'translate-x-0' : '-translate-x-full', sidebarExpanded ? 'md:w-[260px]' : 'md:w-[88px]']" 
       class="fixed inset-y-0 left-0 z-40 bg-white dark:bg-slate-900 border-r border-slate-100 dark:border-slate-800 flex flex-col md:translate-x-0 overflow-visible w-[260px] transition-all duration-300 ease-in-out">
    ...
    <div class="h-20 flex items-center flex-shrink-0 relative" :class="sidebarExpanded ? 'px-6' : 'px-0 justify-center'">
        <a href="{{ route('dashboard') }}" class="flex items-center overflow-hidden">
            <x-application-logo class="h-8 w-auto fill-current text-blue-600 dark:text-blue-500 flex-shrink-0" />
        ...
        <button @click="sidebarExpanded = !sidebarExpanded" 
                class="absolute -right-3 top-7 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 hover:text-blue-600 dark:hover:text-blue-400 rounded-full p-1 shadow-sm hidden md:flex items-center justify-center transition-transform z-50"
    ...
    <div class="p-4 mb-2">
        <div class="flex items-center rounded-2xl" :class="sidebarExpanded ? 'bg-slate-50 dark:bg-slate-800/50 p-3 justify-between' : 'justify-center p-0'">
            <div class="flex items-center" :class="sidebarExpanded ? '' : 'justify-center'">
                <div class="w-10 h-10 rounded-full bg-cyan-100 dark:bg-cyan-900/50 flex items-center justify-center flex-shrink-0 border border-cyan-200 dark:border-cyan-800">
                    <span class="text-cyan-700 dark:text-cyan-400 font-bold text-sm">{{ substr(Auth::user()->name, 0, 1) }}</span>
                </div>
            ...
            <form method="POST" action="{{ route('logout') }}" x-show="sidebarExpanded">
                @csrf
                <button type="submit" class="text-slate-400 hover:text-red-500 hover:bg-white dark:hover:bg-slate-700 rounded-lg p-1.5 transition-colors" title="Log Out">
